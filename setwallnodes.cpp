  ...
  gamma18 = t2*(1.0+3.0*(-ux[k]-uz[k]+ux[k]*ux[k]+uz[k]*uz[k])+9.0*ux[k]*uz[k] - 1.5*uy[k]*uy[k]);
	...
	*/
	...
	else if(mask[k]==1)
	{
		...
		h10[k]=h7[k];
		h8[k]=h9[k];
		//h0[k]+=h[d[0]][1]+h[d[6]][7]+h[d[8]][9]-h2[k]-h8[k]-h10[k];
		
		g2[k]=g1[k];
		g10[k]=g7[k];
		g8[k]=g9[k];
		//g0[k]+=g[d[0]][1]+g[d[6]][7]+g[d[8]][9]-g2[k]-g8[k]-g10[k];
	...
	else if(mask[k]==2)
	{
		...
		h7[k]=h10[k];
		h9[k]=h8[k];
		//h0[k]+=h[d[1]][2]+h[d[7]][8]+h[d[9]][10]-h1[k]-h7[k]-h9[k];

		
		g1[k]=g2[k];
		g7[k]=g10[k];
		g9[k]=g8[k];
		//g0[k]+=g[d[1]][2]+g[d[7]][8]+g[d[9]][10]-g1[k]-g7[k]-g9[k];
