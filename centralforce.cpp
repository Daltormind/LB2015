...
FhC10=(gradCC10-3.0*C[k]/rho[k]*(gradpC10-mu[k]*gradCC10/*+rho[k]*(G[0]+G[1])*/));
 ...
 FgCz=mu[k]*gradCCz;//+rho[k]*G[2];
		...
		*/
		 ...
		 FhU3=(gradCU3-3.0*C[k]/rho[k]*(gradpU3-mu[k]*gradCU3-rho[k]*G[1]));
		 FhU4=(gradCU4-3.0*C[k]/rho[k]*(gradpU4-mu[k]*gradCU4+rho[k]*G[1]));
		 FhU7=(gradCU7-3.0*C[k]/rho[k]*(gradpU7-mu[k]*gradCU7-rho[k]*(G[0]+G[1])));
		 FhU8=(gradCU8-3.0*C[k]/rho[k]*(gradpU8-mu[k]*gradCU8+rho[k]*(G[0]-G[1])));
   		 FhU9=(gradCU9-3.0*C[k]/rho[k]*(gradpU9-mu[k]*gradCU9+rho[k]*(-G[0]+G[1])));
		 FhU10=(gradCU10-3.0*C[k]/rho[k]*(gradpU10-mu[k]*gradCU10+rho[k]*(G[0]+G[1])));
		 
		 FhU5=(gradCU5-3.0*C[k]/rho[k]*(gradpU5-mu[k]*gradCU5-rho[k]*G[2]));
		 FhU6=(gradCU6-3.0*C[k]/rho[k]*(gradpU6-mu[k]*gradCU6+rho[k]*G[2]));
		 FhU11=(gradCU11-3.0*C[k]/rho[k]*(gradpU11-mu[k]*gradCU11-rho[k]*(G[2]+G[1])));
		 FhU12=(gradCU12-3.0*C[k]/rho[k]*(gradpU12-mu[k]*gradCU12+rho[k]*(-G[2]+G[1])));
		 FhU13=(gradCU13-3.0*C[k]/rho[k]*(gradpU13-mu[k]*gradCU13+rho[k]*(G[2]-G[1])));
		 FhU14=(gradCU14-3.0*C[k]/rho[k]*(gradpU14-mu[k]*gradCU14+rho[k]*(G[2]+G[1])));
		 FhU15=(gradCU15-3.0*C[k]/rho[k]*(gradpU15-mu[k]*gradCU15-rho[k]*(G[2]+G[0])));
		 FhU16=(gradCU16-3.0*C[k]/rho[k]*(gradpU16-mu[k]*gradCU16+rho[k]*(G[0]-G[2])));
		 FhU17=(gradCU17-3.0*C[k]/rho[k]*(gradpU17-mu[k]*gradCU17+rho[k]*(-G[0]+G[2])));
		 FhU18=(gradCU18-3.0*C[k]/rho[k]*(gradpU18-mu[k]*gradCU18+rho[k]*(G[0]+G[2])));

  	     FhUx=(gradCUx-3.0*C[k]/rho[k]*(gradpUx-mu[k]*gradCUx-rho[k]*G[0]));
  	     FhUy=(gradCUy-3.0*C[k]/rho[k]*(gradpUy-mu[k]*gradCUy-rho[k]*G[1]));
  	     FhUz=(gradCUz-3.0*C[k]/rho[k]*(gradpUz-mu[k]*gradCUz-rho[k]*G[2]));
		
  	     FgU1=mu[k]*gradCU1+rho[k]*G[0];
    	 FgU2=mu[k]*gradCU2-rho[k]*G[0];
    	 FgU3=mu[k]*gradCU3+rho[k]*G[1];
    	 FgU4=mu[k]*gradCU4-rho[k]*G[1];
      	 FgU7=mu[k]*gradCU7+rho[k]*(G[1]+G[0]);
	 FgU8=mu[k]*gradCU8-rho[k]*(-G[1]+G[0]);
	 FgU9=mu[k]*gradCU9-rho[k]*(-G[0]+G[1]);
	 FgU10=mu[k]*gradCU10-rho[k]*(G[0]+G[1]);
		 
		 FgU5=mu[k]*gradCU5+rho[k]*G[2];
		 FgU6=mu[k]*gradCU6-rho[k]*G[2];
			FgU11=mu[k]*gradCU11+rho[k]*(G[1]+G[2]);
			FgU12=mu[k]*gradCU12+rho[k]*(G[2]-G[1]);
			FgU13=mu[k]*gradCU13+rho[k]*(G[1]-G[2]);
			FgU14=mu[k]*gradCU14-rho[k]*(G[1]+G[2]);
			FgU15=mu[k]*gradCU15+rho[k]*(G[0]+G[2]);
			FgU16=mu[k]*gradCU16+rho[k]*(G[2]-G[0]);
			FgU17=mu[k]*gradCU17+rho[k]*(G[0]-G[2]);
			FgU18=mu[k]*gradCU18-rho[k]*(G[0]+G[2]);

		 FgUx=mu[k]*gradCUx-rho[k]*G[0];
		 FgUy=mu[k]*gradCUy-rho[k]*G[1];
